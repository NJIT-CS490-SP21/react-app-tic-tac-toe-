...
def foo13(data):
    ...
    socketio.emit('newrank', [users, score, data], broadcast=True, include_self=True) 
    return d
if __name__ == "__main__":
    #db.create_all()
    socketio.run(
    app,host=os.getenv('IP', '0.0.0.0'), port=8081 if os.getenv('C9_PORT') else int(os.getenv('PORT', 8081)), debug=True)
